...
const messages = {
  ...
  hasCompany: 'Employee work in company now!',
  dontEmployeeObj: 'Please try to add Employee instance',
  wrongCompanyArgument: 'The company accepts only object with name, owner, maxCompanySize in its argument'
}
const one = 1;
const two = 2;
const thousand = 1000;
...
function Company(company) {
  ...
  if (typeof company !== 'object' || arguments.length > two) {
    return console.log(messages.wrongCompanyArgument);
  }
  ...
  _logs.push(`${this.companyName} was created in ${getDate()}, by ${this.owner}`);
  ...
  this.removeEmployee = function(id) {
    if (!isNaN(parseInt(id)) && isFinite(id)) {
      _logs.push(`${_currentEmployees[id].name} ends working in ${this.companyName} in ${getDate()}`);
  ...
  this.getFormattedListOfEmployees = function() {
    return _currentEmployees.map(function(emp) {
      return `${emp.name} - works in ${emp.company} ${emp.getWorkTimeInSeconds()} seconds`;
    });
  };
...
function Employee(employee) {
  ...
  this.hire = function(currentCompanyName) {
    if (this.company) {
      return console.log(messages.hasCompany);
    } else {
      _startDate = getDate();
      this.company = currentCompanyName;
      _logs.push(`${this.name} is hired to ${this.company} in ${getDate()}`);
    }
  };

  this.fire = function() {
    _finalDate = getDate();
    _logs.push(`${this.name} is fired from ${this.company} in ${getDate()}`);
